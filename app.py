...
def generate_video(input_text, detail_level, language, accuracy, painting_model, painting_option, openai_api_key):
    search_result, explanation, prompts, titles = main(input_text, detail_level, language, accuracy, painting_model, painting_option, openai_api_key)
    print(search_result)
    # セッション状態に返されたテキストを保存
...
def main_page():
    ...
    if st.button("Generate"):
        ...
        painting_model = st.session_state.get("painting_model")
        openai_api_key = st.session_state.get("openai_api_key")
        generate_video(input_text, detail_level, language, accuracy, painting_model, painting_option, openai_api_key)
...
def sub_page():
    ...
    PAINTING_MODEL = st.sidebar.selectbox("model", ["dall-e-2", "dall-e-3"])
    ...
    OPENAI_API_KEY = st.sidebar.text_input("OpenAI API key", placeholder="Enter your OpenAI API key")

    # Optionをセッション状態に保存
    st.session_state["detail_level"] = DETAIL_LEVEL
    st.session_state["language"] = LANGUAGE
    st.session_state["accuracy"] = ACCURACY
    st.session_state["painting_model"] = PAINTING_MODEL
...
def main_video_page():
    ...
    with right_column:
        ...
        st.markdown("<h1 style='font-size: 24px;'>One-Minute.ai</h1>", unsafe_allow_html=True)
        st.markdown("<p style='font-size: 18px;'>One Minute Video to Grasp Anything.</p>", unsafe_allow_html=True)
...
def sub_video_page():
    ...
    with tab2:
        ...
        if st.button("Generate"):
            ...
            painting_model = st.session_state.get("painting_model")
            openai_api_key = st.session_state.get("openai_api_key")
            generate_video(txt, detail_level, LANGUAGE, accuracy, painting_model, painting_option, openai_api_key)
            st.session_state["page"] = "video"
            st.experimental_rerun()
