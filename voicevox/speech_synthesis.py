from voicevox.voice import pyopenjtalk_synthesize as voicevox
...
def create_sound_files_srt_files(Section_List, Keywords):
    ...
    for i, section in enumerate(Section_List):
        for j, sentence in enumerate(section):
            ...
            sentence = re.sub(r"　", "", sentence)
            time = voicevox(sentence, f"/tmp/{i}_{j}.wav", speed=1.2)
            end = start + time
    ...
    with open("/tmp/output.srt", "w") as f:
        f.write(srt.compose(srts))
        
    
    with open("/tmp/keywords.srt", "w") as f:
...
def concat_sound_file(Section_List):
    ...
    for i, section in enumerate(Section_List):
        for j, _ in enumerate(section):
            afes.append(AudioSegment.from_wav(f"/tmp/{i}_{j}.wav"))
...
def combine_video_srt():
    video = VideoFileClip("/tmp/output.mp4")
    ...
    # SubtitlesClipを生成
    srt = SubtitlesClip("/tmp/output.srt", make_textclip=generator)
    keywords_srt = SubtitlesClip("/tmp/keywords.srt", make_textclip=generator_keywords)
    ...
    final_video.write_videofile("/tmp/output_with_srt.mp4", codec="libx264", fps=24)
