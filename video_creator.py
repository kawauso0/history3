...
def generate_image(prompt, painting_model):
    try:
...
def generate_image_with_retry(prompt, painting_model, retries=5, delay=12):
    for attempt in range(retries):
        image_url = generate_image(prompt, painting_model)
        if image_url:
            return image_url
